					...
					switch (target)
					{
						case IpcRoutes.Agent:
						{
							using (var scope = _browserService.CreateScope(message))
							{
								...
								try
								{
									@params = await _codeStreamAgent.SendAsync<JToken>(
										message.Method,
										message.Params
									);
								}
						...
						case IpcRoutes.Host:
						{
							switch (message.Method)
							{
								case EditorCopySymbolRequestType.MethodName:
								{
									...
									{
										var request =
											message.Params?.ToObject<EditorCopySymbolRequest>();

										var response = await _symbolService.CopySymbolAsync(
											request,
								...
								case EditorRevealSymbolRequestType.MethodName:
								{
									...
									{
										var request =
											message.Params?.ToObject<EditorRevealSymbolRequest>();

										await _symbolService.RevealSymbolAsync(
											$"{request.CodeNamespace}.{request.CodeFunction}",
											CancellationToken.None
										);
								...
								case ShellPromptFolderRequestType.MethodName:
								{
									using (var scope = _browserService.CreateScope(message))
									{
										...
										try
										{
											...
											var request =
												message.Params?.ToObject<ShellPromptFolderRequest>();
								...
								{
									var @params =
										message.Params?.ToObject<WebviewDidChangeContextNotification>();
									if (@params != null)
									{
										...
										if (panelStack != null)
										{
											...
											_eventAggregator.Publish(
												new MarkerGlyphVisibilityEvent
												{
													IsVisible = !visible
												}
											);
										}
										_webviewUserSettingsService.SaveContext(
								...
								case CompareMarkerRequestType.MethodName:
								{
									using (_browserService.CreateScope(message))
									{
										try
										{
											var marker = message.Params?[
												"marker"
											]?.ToObject<CsMarker>();
											var documentFromMarker =
												await _codeStreamAgent.GetDocumentFromMarkerAsync(
													new DocumentFromMarkerRequest(marker)
												);

											var fileUri =
												documentFromMarker.TextDocument.Uri.ToUri();
											...
											var wpfTextView =
												await _ideService.OpenEditorAtLineAsync(
													fileUri,
													documentFromMarker.Range,
													true
												);
											...
											{
												var text =
													wpfTextView.TextBuffer.CurrentSnapshot.GetText();
												var span = wpfTextView.ToSpan(
													documentFromMarker.Range
												);
												if (span.HasValue)
												{
										...
										catch (Exception ex)
										{
											Log.Error(
												ex,
												nameof(CompareMarkerRequestType.MethodName)
											);
										}
									}
									break;
									...
									using (_browserService.CreateScope(message))
									{
										try
										{
											...
											if (wpfTextView != null)
											{
												...
												if (span.HasValue)
												{
													wpfTextView.TextBuffer.Replace(
														span.Value,
														documentFromMarker.Marker.Code
													);
												}
										...
										catch (Exception ex)
										{
											Log.Error(
												ex,
												nameof(ApplyMarkerRequestType.MethodName)
											);
										}
									}
									break;
								...
								case BootstrapInHostRequestType.MethodName:
								{
									try
									{
										...
										using (var scope = _browserService.CreateScope(message))
										{
											try
											{
												@params = await _codeStreamAgent.GetBootstrapAsync(
													_codeStreamSettingsManager.GetSettings(),
													_sessionService.State,
													_sessionService.IsReady
												);
											}
											catch (Exception ex)
											{
									...
									using (_browserService.CreateScope(message))
									{
										if (_authenticationServiceFactory != null)
										{
											var authenticationService =
												_authenticationServiceFactory.Create();
											if (authenticationService != null)
											{
												var @params =
													message.Params?.ToObject<LogoutRequest>();
												var reason =
													@params?.Reason
													== LogoutReason1.Reauthenticating
								...
								case GetActiveEditorContextRequestType.MethodName:
								{
									...
									{
										scope.FulfillRequest(
											new GetActiveEditorContextResponse(
												_editorService.GetEditorContext()
											).ToJToken()
										);
									}
									break;
								...
								case EditorSelectRangeRequestType.MethodName:
								{
									using (var scope = _browserService.CreateScope(message))
									{
										...
										{
											var @params =
												message.Params?.ToObject<EditorSelectRangeRequest>();
											if (@params != null)
											{
												...
												var editorResponse =
													await _ideService.OpenEditorAtLineAsync(
														uri,
														@params.Selection.ToRange(),
														true
													);
												if (editorResponse != null)
												{
													result = new ActiveTextEditor(
														editorResponse,
														uri.NormalizePath(),
														uri,
														editorResponse.TextSnapshot?.LineCount
													).SelectRange(
														@params.Selection,
														@params.PreserveFocus == false
													);
													if (!result)
													{
										...
										catch (Exception ex)
										{
											Log.Warning(
												ex,
												nameof(EditorSelectRangeRequestType.MethodName)
											);
										}
										scope.FulfillRequest(
											new EditorSelectRangeResponse
											{
												Success = result
											}.ToJToken()
										);
								...
								case EditorHighlightRangeRequestType.MethodName:
								{
									using (var scope = _browserService.CreateScope(message))
									{
										bool result = false;
										var @params =
											message.Params.ToObjectSafe<EditorHighlightRangeRequest>();
										if (@params != null)
										{
											var activeTextView =
												_editorService.GetActiveTextEditorFromUri(
													@params.Uri.ToUri()
												);
											if (activeTextView != null)
											{
												...
												result = activeTextView.Highlight(
													@params.Range,
													@params.Highlight
												);
												if (!result)
												{
													Log.Verbose(
														$"{nameof(EditorHighlightRangeRequestType)} result is false"
													);
												}
											}
										}
										scope.FulfillRequest(
											new EditorHighlightRangeResponse
											{
												Success = result
											}.ToJToken()
										);
								...
								case EditorRevealRangeRequestType.MethodName:
								{
									using (var scope = _browserService.CreateScope(message))
									{
										...
										var @params =
											message.Params?.ToObject<EditorRevealRangeRequest>();
										if (@params != null)
										{
											openEditorResult =
												await _ideService.OpenEditorAndRevealAsync(
													@params.Uri.ToUri(),
													@params.Range?.Start?.Line,
													atTop: @params.AtTop,
													focus: @params.PreserveFocus == false
												);
											if (openEditorResult?.Success != true)
											{
												Log.Verbose(
													$"{nameof(EditorRevealRangeRequestType)} result is false"
												);
											}
										}
										scope.FulfillRequest(
											new EditorRevealRangeResponse
											{
												Success = openEditorResult?.Success == true
											}.ToJToken()
										);
									}
									break;
								...
								{
									var @params =
										message.Params?.ToObject<EditorScrollToNotification>();
									...
									System.Windows.Application.Current.Dispatcher.Invoke(
										() =>
										{
											...
											catch (Exception ex)
											{
												Log.Warning(
													ex,
													nameof(EditorRevealRangeRequestType)
												);
											}
										},
...
#pragma warning restore VSTHRD001
								...
								case InsertTextRequestType.MethodName:
								{
									using (var scope = _browserService.CreateScope(message))
									{
										...
										{
											var @params =
												message.Params?.ToObject<InsertTextRequest>();
											if (@params != null)
											{
												var documentFromMarker =
													await _codeStreamAgent.GetDocumentFromMarkerAsync(
												...
												if (documentFromMarker != null)
												{
													...
													{
														Log.Debug(
															$"{nameof(InsertTextRequestType)} could not open editor"
														);
													}
													else
													{
														var span = openEditorResult.ToStartLineSpan(
															documentFromMarker.Range.Start.Line
														);
														if (span != null)
														{
															using (
																var edit =
																	openEditorResult.TextBuffer.CreateEdit()
															)
															{
																edit.Insert(
																	span.Value.Start,
														...
														{
															Log.Debug(
																$"Could not locate Span. Range={documentFromMarker.Range}"
															);
														}
													}
												}
								...
								case ReloadRequestType.MethodName:
								{
									var languageServerClientManager =
										_componentModel.GetService<ILanguageServerClientManager>();
								...
								case ReloadWebviewRequestType.MethodName:
								{
									using (_browserService.CreateScope(message))
									{
										_webviewUserSettingsService.TryClearContext(
											_sessionService.SolutionName,
											_sessionService.TeamId
										);
										_browserService.ReloadWebView();
									}
									break;
								...
								case UpdateServerUrlRequestType.MethodName:
								{
									...
									using (var scope = _browserService.CreateScope(message))
									{
										...
										{
											var @params =
												message.Params?.ToObject<UpdateServerUrlRequest>();
											Log.Debug(
												$"{nameof(UpdateServerUrlRequestType)} ServerUrl={@params?.ServerUrl}"
											);

											if (
												(@params?.CopyToken ?? false)
											...
											using (
												var settingsScope = SettingsScope.Create(
													_codeStreamSettingsManager,
													true
												)
											)
											{
												settingsScope.CodeStreamSettingsManager.ServerUrl =
													@params?.ServerUrl;
												settingsScope
													.CodeStreamSettingsManager
													.DisableStrictSSL =
													@params?.DisableStrictSSL ?? false;
											}

											await _codeStreamAgent.SetServerUrlAsync(
												@params?.ServerUrl,
										...
										scope.FulfillRequest(
											new UpdateServerUrlResponse().ToJToken()
										);
									}
									break;
								}
								...
								{
									var @params =
										message.Params?.ToObject<PullRequestShowDiffRequest>();
									await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync(
										CancellationToken.None
									);
									...
									using (var scope = _browserService.CreateScope(message))
									{
										...
										{
											var leftContent =
												await _codeStreamAgent.GetFileContentsAtRevisionAsync(
													@params.RepoId,
													@params.FilePath,
													@params.BaseSha
												);
								...
								case ReviewShowDiffRequestType.MethodName:
								{
									...
									using (var scope = _browserService.CreateScope(message))
									{
										...
										{
											var reviewContents =
												await _codeStreamAgent.GetReviewContentsAsync(
													@params.ReviewId,
													@params.Checkpoint,
													@params.RepoId,
													@params.Path
											...
											{
												var review = await _codeStreamAgent.GetReviewAsync(
													@params.ReviewId
												);
												if (review != null)
												{
													...
													title =
														$"{@params.Path} @ {review.Review.Title.Truncate(25)}{title}";

													var leftData = new FeedbackRequestDiffUri(
														@params.ReviewId,
														@params.Checkpoint?.ToString()
															?? "undefined",
														@params.RepoId,
														"left",
														@params.Path
								...
								{
									var @params =
										message.Params?.ToObject<ReviewShowLocalDiffRequest>();
									...
									using (var scope = _browserService.CreateScope(message))
									{
										...
										{
											var reviewContents =
												await _codeStreamAgent.GetReviewContentsLocalAsync(
													@params.RepoId,
													@params.Path,
													@params.EditingReviewId,
													@params.BaseSha,
													@params.IncludeSaved.HasValue
													&& @params.IncludeSaved.Value
														? "saved"
														: @params.IncludeStaged.HasValue
														&& @params.IncludeStaged.Value
															? "staged"
															: "head"
												);
											if (reviewContents != null)
											{
												...
												_ideService.DiffTextBlocks(
													...
													reviewContents.Left,
													reviewContents.Right,
													$"{@params.Path.Truncate(25)} review changes"
												);
											}
										}
										scope.FulfillRequest();
								...
								{
									Log.Warning(
										$"Unhandled Target={target} Method={message.Method}"
									);
									break;
								}
							}
