...
namespace CodeStream.VisualStudio.Shared.LanguageServer
{
	public class CustomMessageHandler : IDisposable
	{
			...
			_documentMarkerChangedSubscription = _documentMarkerChangedSubject
				...
				.Subscribe(e =>
				{
					_eventAggregator.Publish(
						new DocumentMarkerChangedEvent { Uri = e.Uri.ToUri() }
					);
				});
		...
		public async System.Threading.Tasks.Task OnOpenUrlAsync(
			JToken e,
			JToken someOtherPropThatNeedsToBeHereForRequests
		)
		{
			using (
		...
		public void OnDidChangeData(JToken e)
		{
			...
			if (type?.Value<string>() == "preferences")
			{
				...
				{
					_userPreferencesChangedSubject.OnNext(
						new UserPreferencesChangedSubjectArgs(preferences.Data)
					);
				}
			}
			...
			switch (@params.Status)
			{
				...
				case ConnectionStatus.Reconnecting:
					BrowserService.EnqueueNotification(
						new DidChangeConnectionStatusNotificationType(@params)
					);
					break;
		...
		public void OnDidChangeApiVersionCompatibilityNotification(JToken e)
		{
			using (
				Log.CriticalOperation(
					$"{nameof(OnDidChangeApiVersionCompatibilityNotification)} Method={DidChangeApiVersionCompatibilityNotificationType.MethodName}",
					Serilog.Events.LogEventLevel.Information
				)
			)
			{
					...
					ThreadHelper.JoinableTaskFactory.Run(
						async delegate
						{
							...
							catch (Exception ex)
							{
								Log.Error(
									ex,
									nameof(OnDidChangeApiVersionCompatibilityNotification)
								);
							}
						}
					);
				...
				BrowserService.EnqueueNotification(
					new DidChangeApiVersionCompatibilityNotificationType(e)
				);
			}
		}
		...
		{
			public DocumentMarkerChangedSubjectArgs(string uri) => Uri = uri;
		...
		public void OnDidChangeDocumentMarkers(JToken e)
		{
			...
			BrowserService.EnqueueNotification(
				new DidChangeDocumentMarkersNotificationType(@params)
			);
			...
			if (@params.Reason == ChangeReason.Codemarks)
			{
				...
				_documentMarkerChangedSubject.OnNext(
					new DocumentMarkerChangedSubjectArgs(@params.TextDocument.Uri)
				);
			}
		}
		...
		public void OnDidChangeVersionCompatibility(JToken e)
		{
			using (
				Log.CriticalOperation(
					$"{nameof(OnDidChangeVersionCompatibility)} Method={DidChangeVersionCompatibilityNotificationType.MethodName}",
					Serilog.Events.LogEventLevel.Information
				)
			)
			{
				...
				BrowserService.EnqueueNotification(
					new DidChangeVersionCompatibilityNotificationType(e)
				);
			}
		}
		...
		public async System.Threading.Tasks.Task OnDidLogoutAsync(JToken e)
		{
			try
			{
				...
				using (
					Log.CriticalOperation(
						$"{nameof(OnDidLogin)} Method={DidLogoutNotificationType.MethodName} Reason={@params?.Reason}",
						Serilog.Events.LogEventLevel.Information
					)
				)
				{
		...
		public void OnDidLogin(JToken e)
		{
			using (
				Log.CriticalOperation(
					$"{nameof(OnDidLogin)} Method={DidLoginNotificationType.MethodName}",
		...
		public async System.Threading.Tasks.Task RestartRequiredAsync()
		{
			using (
				Log.CriticalOperation(
					$"{nameof(RestartRequiredAsync)} Method={RestartRequiredNotificationType.MethodName}",
					Serilog.Events.LogEventLevel.Debug
				)
			)
			{
		...
		public async System.Threading.Tasks.Task MaintenanceModeAsync(JToken e)
		{
			using (
				Log.CriticalOperation(
					$"{nameof(MaintenanceModeAsync)} Method={DidEncounterMaintenanceModeNotificationType.MethodName}",
					Serilog.Events.LogEventLevel.Debug
				)
			)
			{
		...
		public async System.Threading.Tasks.Task SetEnvironmentAsync(JToken e)
		{
			using (
				Log.CriticalOperation(
					$"{nameof(SetEnvironmentAsync)} Method={DidSetEnvironmentNotificationType.MethodName}",
					Serilog.Events.LogEventLevel.Debug
				)
			)
			{
				try
				{
					_settingsServiceFactory
						.GetOrCreate()
						.SetEnvironment(e.ToObject<CodeStreamEnvironmentInfo>());
				}
				catch (Exception ex)
				{
		...
		public async System.Threading.Tasks.Task DidChangeServerUrlAsync(JToken e)
		{
			using (
				Log.CriticalOperation(
					$"{nameof(DidChangeServerUrlAsync)} Method={DidChangeServerUrlNotificationType.MethodName}",
					Serilog.Events.LogEventLevel.Debug
				)
			)
			{
		...
		public System.Threading.Tasks.Task<ResolveStackTracePathsResponse> OnResolveStackTracePaths(
			JToken e,
			JToken someOtherPropThatNeedsToBeHereForRequests
		)
		{
		...
		public void OnDidResolveStackTraceLine(JToken e)
		{
			using (
				Log.CriticalOperation(
					$"{nameof(OnDidResolveStackTraceLine)} Method={DidResolveStackTraceLineNotificationType.MethodName}",
					Serilog.Events.LogEventLevel.Debug
				)
			)
			{
				...
				{
					BrowserService.EnqueueNotification(
						new DidResolveStackTraceLineNotificationType(e)
					);
				}
				catch (Exception ex)
				{
		...
		)
		{
			try
			{
				...
				var authenticationServiceFactory =
					componentModel.GetService<IAuthenticationServiceFactory>();
