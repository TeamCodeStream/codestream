...
namespace CodeStream.VisualStudio.Shared.Models
{
	...
	{
		[JsonConstructor]
		public EditorSelection(Position cursor, Position start, Position end)
			: this(cursor, new Range { Start = start, End = end }) { }
	...
	{
		public static Range ToRange(this EditorSelection selection) =>
			selection != null ? selection.ToRange() : null;
	}
	...
	public class DocumentMarker : CsEntity, ICSMarkerIdentifier
	{
		...
		public bool Pinned
		{
			get { return Codemark?.Pinned == true; }
		}

		[JsonIgnore]
		public string Status
		{
			get { return Codemark?.Status ?? "open"; }
		}
