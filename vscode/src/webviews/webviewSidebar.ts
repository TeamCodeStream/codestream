			...
			fs.readFile(Container.context.asAbsolutePath("webview.html"), "utf8", (err, data) => {
				if (err) {
					...
					if (
						...
						Container.telemetryOptions.applicationId
					) {
						try {
							...
							if (browserScript) {
								const browser = fs
									...
									.replace(/{{applicationID}}/g, Container.telemetryOptions.applicationId!)
									// agentID === applicationID
									.replace(/{{agentID}}/g, Container.telemetryOptions.applicationId!)
